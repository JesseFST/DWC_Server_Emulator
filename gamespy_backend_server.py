...
#    Copyright (C) 2019 TwilightGamez, LLC. 
#
...
class GameSpyBackendServer(object):
	...
	def get_token(self, filters):
		...
		# dwc_mver = 3 and dwc_pid != 4 and maxplayers = 3 and numplayers < 3
		# and dwc_mtype = 0 and dwc_mresv != dwc_pid and
		# (((20=auth)AND((1&mskdif)=mskdif)AND((14&mskstg)=mskstg)))
		#
		# Example with OR from Mario Kart Wii:
		# dwc_mver = 90 and dwc_pid != 1 and maxplayers = 11 and numplayers <
		# 11 and dwc_mtype = 0 and dwc_hoststate = 2 and dwc_suspend = 0 and
		# (rk = 'vs_123' and (ev > 4263 or ev <= 5763) and p = 0)
		i = 0
		start = i
		special_chars = "_"
	...
	def translate_expression(self, filters):
		...
		while len(filters) > 0:
			...
			if token_type == TokenType.TOKEN:
				# Python uses == instead of = for comparisons, so replace it
				# with the proper token for compilation.
				if token == "=":
					token = "=="
	...
	def validate_ast(self, node, num_literal_only):
		...
		# Never run the expression received from the client before running this
		# function on the expression first.
		#print type(node)
		...
		if isinstance(node, ast.Num):
			...
			for op in node.ops:
				...
				# Restrict "is", "is not", "in", and "not in" python comparison
				# operators.
				# These are python-specific and the games have no way of
				# knowing what they are, so there's no reason
				# to keep them around.
	...
	def find_servers(self, gameid, filters, fields, max_count):
		...
		for server in self.server_list[gameid]:
			...
			if filters != "":
				...
				# Always run validate_ast over the entire AST before evaluating
				# anything.  eval() is dangerous to use on
				# unsanitized inputs.  The validate_ast function has a fairly
				# strict whitelist so it should be safe in what
				# it accepts as valid.
				...
				if valid_filter == False:
					...
					# little time but it shouldn't be all that
					# big of a difference, I think.  It takes about 0.0004
					# seconds per server to determine whether or not it's a
					# match on my computer.  Usually there's a low max_servers
					# set when the game searches for servers, so assuming
					# something like the game is asking for 6 servers, it would
					# take about 0.0024 seconds total.  These times
					# will obviously be different per computer.  It's not
					# ideal, but it shouldn't be a huge bottleneck.
					# A possible way to speed it up is to make validate_ast
					# also evaluate the expressions at the same time as it
					# validates it.
					result = eval(q)
			else:
		...
		for server in matched_servers:
			...
			for field in fields:
				...
				if field in server:
					...
					# Return a dummy value.  What's the normal behavior of the
					# real server in this case?
					requested[field] = ""
	...
	def update_server_list(self, gameid, session, value, console):
		# Make sure the user isn't hosting multiple servers or there isn't some
		# left over server information that
			...
			for server in self.server_list[gameid]:
				...
				if server['publicip'] == publicip:
					...
					if localport == 0 and best_match == None:
						...
						# the same IP, this might be safe.
						# The server wasn't verified to be the *correct*
						# server, but it's on the same IP so it
						# has a chance of being correct.  At least make an
