...
class Session(LineReceiver):
		...
		try:
			...
			while len(self.buffer) > 0:
				...
				if packet[2] == '\x00': # Server list request
					...
					# This code is so...  not python.  The C programmer in me
					...
					#print "%02x %02x %08x" % (list_version, encoding_version,
					#game_version)
					#print "%s" % query_game
					#print "%s" % game_name
					#print "%s" % challenge
	...
	def generate_server_list_data(self, address, fields, server_info, finalize=False):
		...
		if len(server_info) > 0:
			# Start server loop here instead of including all of the fields and
			# stuff again
			flags = 0
			if len(server_info) != 0:
	...
	def find_server(self, query_game, filter, fields, max_servers, game_name, challenge):
		...
		for i in range(0, len(self.server_list)):
			...
			if server and fields and 'requested' in server and server['requested'] == {}:
				# If the requested fields weren't found then don't return a
				# server.
	...
	def forward_data_to_client(self, data, forward_client):
		...
		if server['publicip'] == ip and server['publicport'] == str(forward_client[1]):
			if forward_client[1] == 0 and 'localport' in server:
				# No public port returned from client, try contacting on the
				# local port.
				forward_client = (forward_client[0], int(server['localport']))

			# Send command to server to get it to connect to natneg
			cookie = int(utils.generate_random_hex_str(8), 16) # Quick and lazy way to get a random 32bit integer.  Replace with something
															   # else later

			#if (len(data) == 24 and bytearray(data)[0:10] == bytearray([0x53,
			#0x42, 0x43, 0x4d, 0x03, 0x00, 0x00, 0x00, 0x01, 0x04])) or
			#(len(data) == 40 and bytearray(data)[0:10] == bytearray([0x53,
			#0x42, 0x43, 0x4d, 0x0b, 0x00, 0x00, 0x00, 0x01, 0x04])):
			if self.own_server == None and len(data) >= 16 and bytearray(data)[0:4] in (bytearray([0xbb, 0x49, 0xcc, 0x4d]), bytearray([0x53, 0x42, 0x43, 0x4d])):
				# Is the endianness the same between the DS and Wii here?  It
				# seems so but I'm not positive.
				...
				#
				#     self.log(logging.DEBUG, "Adding %d to natneg server list:
				#     %s" % (natneg_session, own_server))
				#     self.server_manager.add_natneg_server(natneg_session,
				#     own_server) # Store info in backend so we can get it
				#     later in natneg
			...
			if self.qr != None:
				...
				# In case we can't contact the QR server, just try sending the
				# packet directly.
				# This isn't standard behavior but it can work in some
				# instances.
