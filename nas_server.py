...
# if a game from this list requests a file listing, the server will return that
# only one exists and return a random one
...
class NasHTTPServerHandler(BaseHTTPServer.BaseHTTPRequestHandler):
			...
			if self.path == "/ac":
				...
				if action == "acctcreate":
					...
					if self.server.db.is_dolphin(post):
						...
						logger.log(logging.DEBUG, "login denied for banned Mii or device name " + str(post))
						ret = {
							"datetime": time.strftime("%Y%m%d%H%M%S"),
							"returncd": "3918",
							"locator": "gamespy.com",
							"retry": "1",
							"reason": "Mii or device name is black listed"
						}
						...
						post["region"] = unicode(post["region"], errors='ignore')

						# abuse detection
						if len(post["region"]) > 2:
							logger.log(logging.DEBUG, "Login denied because region data is invalid " + str(post))
							ret = {
								"datetime": time.strftime("%Y%m%d%H%M%S"),
								"returncd": "2222", 
								"locator": "gamespy.com", 
					...
					if 'svc' in post:
						if post["svc"] in ("9000", "9001"): # DLC host = 9000
							...
							# Brawl has 2 host headers which Apache chokes on,
							# so only return the first one or else it won't
							# work
	...
	def str_to_dict(self, s):
		...
		for k, v in ret.iteritems():
			try:
				...
				# For the most part it's not important since it's mostly used
				# for the devname/ingamesn fields.
	...
	# custom selection for generation 5 mystery gifts, so that the random or
	# data-based selection still works properly
