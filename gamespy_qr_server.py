...
class GameSpyQRServer(object):
	...
	def __init__(self):
		...
		# keys associated with their game IDs.
		...
		#self.log(logging.DEBUG, address, session_id, "Generated list of secret
		#game keys...")
	...
	def start(self):
		try:
			...
			# Start QR server
			address = ('0.0.0.0', 27900)  # accessible to outside connections (use this if you don't know what you're
										  # doing)
	...
	def update_server_list(self, session_id, k):
		if "statechanged" in k and k['statechanged'] == "2": # Close server
			...
			# dwc_mtype = 2 is used when hosting a friends only game (possibly
			# other uses too).
	...
	def handle_packet(self, socket, recv_data, address):
		...
		# - The first byte from the client (or third byte from the server) is a
		# command.
		# - Bytes 2 - 5 from the client is some kind of ID.  This will have to
		# be inspected later.  I believe it's a
		# session-like ID because the number changes between connections.
		# Copying the client's ID might be enough.
		#
		...
		# R2 contains a pointer to some unknown integer that gets written after
		# the command.
		#
		# - Commands
		...
		#           Sends back base64 of RC4 encrypted string that was gotten
		#           from the server's 0x01.
		#
		#       0x03 - Send client state?  (Tetris DS overlay 10 @ 216DA30)
		#           Data sent:
		#           1) Loop for each localip available on the system, write as
		#           localip%d\x00(local ip)
		#           2) localport\x00(local port)
		#           3) natneg (either 0 or 1)
		#           4) ONLY IF STATE CHANGED: statechanged\x00(state) (Possible
		#           values: 0, 1, 2, 3)
		...
		#
		#       0x07 - Unknown, related to server's 0x06 (returns value sent
		#       from server)
		#
		...
		#           identifier for the data being sent,
		#           seeing how the server can send the same IP information many
		#           times in a row.  If the IP information has
		#           already been parsed then it doesn't waste time handling it.
		#
		#           After that is a "SBCM" section which is 0x14 bytes in
		#           total.
		#           SBCM information gets parsed at 2141A0C in Tetris DS
		#           overlay 10.
		...
		#           Gets sent after receiving 0x01 from the client.  So, server
		#           0x01 -> client 0x01 -> server 0x0a.
		...
		if recv_data[0] == '\x00': # Query
			...
			for i in range(0, len(d), 2):
				#self.log(logging.DEBUG, address, session_id, "%s = %s" %
				#(d[i], d[i+1]))
				k[d[i]] = d[i + 1]
			...
			if self.sessions[session_id].playerid == 0 and "dwc_pid" in k:
				# Get the player's id and then query the profile to figure out
				# what console they are on.
				# The endianness of some server data depends on the endianness
				# of the console, so we must be able
				# to account for that.
			...
			if 'publicip' in k and k['publicip'] == "0": #and k['dwc_hoststate'] == "2": # When dwc_hoststate == 2 then it doesn't send
														 #an IP, so calculate it ourselves
	...
	def keepalive_check(self):
		#self.log(logging.DEBUG, None, session_id, "Keep alive check on %d
		...
		for session_id in self.sessions:
			...
			timeout = 68 # Remove clients that haven't responded in x seconds
