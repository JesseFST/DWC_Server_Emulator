...
class GameSpyNatNegServer(object):
	...
	def start(self):
		try:
			# Start natneg server
			address = ('0.0.0.0', 27901)  # accessible to outside connections (use this if you don't know what you're
										  # doing)
	...
	def handle_packet(self, recv_data, addr):
		...
		if recv_data[7] == '\x00':
			...
			output += bytearray([0xff, 0xff, 0x6d, 0x16, 0xb5, 0x7d, 0xea]) # Checked with Tetris DS, Mario Kart DS, and Metroid Prime Hunters, and this
																			# seems to be the standard response to 0x00
			...
			for client in self.session_list[session_id]:
				if self.session_list[session_id][client]['connected'] == False: # and self.session_list[session_id][client]['localaddr'][1] != 0:
					...
					#if self.session_list[session_id][client]['serveraddr'] ==
					#None:
					...
					#(self.session_list[session_id][client_id]['addr'])))
					...
					#if self.session_list[session_id][client_id]['serveraddr']
					#== None:
					...
					output[7] = 0x05
					#self.write_queue.put((output,
					#(self.session_list[session_id][client]['addr'])))
			...
			# Only seems to be used in very few DS games (namely, Pokemon
