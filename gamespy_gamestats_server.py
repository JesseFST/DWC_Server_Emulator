...
class Gamestats(LineReceiver):
	...
	def perform_setpd(self, data_parsed):
		...
		msg = gs_query.create_gamespy_message([('__cmd__', "setpdr"),
			('__cmd_val__', 1),
			('lid', self.lid),
			('pid', self.profileid),
			('mod', int(time.time())),])
		...
		if len(data) < length:
			# The packet isn't complete yet, keep loop until we get the entire
			# packet.
			# The length entire packet SHOULD always be greater than the data
			# field, so this check should be fine.
			return
