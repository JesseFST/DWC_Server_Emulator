...
class StorageHTTPServer(BaseHTTPServer.HTTPServer):
	def __init__(self, server_address, RequestHandlerClass):
		...
		self.create_or_alter_table_if_not_exists('g1443_bbdx_player',
			...
			[PK,         'INT'],
			['int',      'int'])
		self.create_or_alter_table_if_not_exists('g1443_bbdx_info',
		...
		# Mario Kart Wii
		self.create_or_alter_table_if_not_exists('g1687_FriendInfo',
		...
		self.create_or_alter_table_if_not_exists('g2050_contest',
			...
			[PK,         'INT',     'INT',  'INT'],
			['int',      'int',     'int',  'int'])
		self.create_or_alter_table_if_not_exists('g2050_contest_eu',
			['recordid', 'ownerid', 'm_no', 'm_file_id'],
			[PK,         'INT',     'INT',  'INT'],
			['int',      'int',     'int',  'int'])
		self.create_or_alter_table_if_not_exists('g2050_contest_us',
			['recordid', 'ownerid', 'm_no', 'm_file_id'],
			[PK,         'INT',     'INT',  'INT'],
			['int',      'int',     'int',  'int'])
		...
		self.create_or_alter_table_if_not_exists('g2649_bbdx_player',
			...
			[PK,         'INT'],
			['int',      'int'])
		self.create_or_alter_table_if_not_exists('g2649_bbdx_info',
			['serialid', 'stat', 'message'],
			['INT',      'INT',  'TEXT'],
			['int',      'int',  'unicodeString'])
		...
		# Playground
		self.create_or_alter_table_if_not_exists('g2999_tblRegionInfo',
			['recordid', 'region', 'allowed_regions', 'min_ratings'],
			[PK,         'INT',    'INT',             'INT'],
			['int',      'byte',   'int',             'int'])

		# Super Smash Bros.  Brawl
		self.create_or_alter_table_if_not_exists('g1658_submit',
			['recordid', 'ownerid', 'data'],
			[PK,         'INT',     'INT'],
			['int',      'int',     'int'])
		self.create_or_alter_table_if_not_exists('g1658_watching',
			['recordid', 'ownerid', 'data'],
			[PK,         'INT',     'INT'],
			['int',      'int',     'int'])

		# Trackmania Wii
		self.create_or_alter_table_if_not_exists('g2793_player',
		...
		self.create_or_alter_table_if_not_exists('g2793_solo',
		...
		self.create_or_alter_table_if_not_exists('g2793_custom',
		...
		self.create_or_alter_table_if_not_exists('g2793_customDLC',
		...
		for t in tabledata:
			cursor.execute("PRAGMA table_info(%s)" % t[0]) # yeah I know but parameters don't work in pragmas, and inserting table names
														   # like that should be safe
	...
	def create_or_alter_table_if_not_exists(self, table, columns, sqlTypes, sakeTypes):
		# this is certainly not the most efficient way to create a table, but
		# it should work and makes that mess in the init function more readable
...
class StorageHTTPServerHandler(BaseHTTPServer.BaseHTTPRequestHandler):
	...
	def parse_filter(self, table, filter):
		# I think I need to read up on how to properly parse SQL-like data, but
		# this should do for the stuff I've seen from games so far.
		out = ''
		...
		for f in filter:
			if f in self.server.tables[table]:
				...
				# try to make the output still valid by removing trailing
				# connecting tokens
	...
	def do_POST(self):
		...
		if self.path == "/SakeStorageServer/StorageServer.asmx" or self.path == "/SakeStorageServer/StorageServer.aspx":
			...
			if shortaction == 'SearchForRecords' or shortaction == 'GetMyRecords' or shortaction == 'GetSpecificRecords':
				...
				# build SELECT statement, yes I know one shouldn't do this but
				# I cross-checked the table name and all the columns above so
				# it should be fine
				statement = 'SELECT '
				...
				if shortaction == 'SearchForRecords':
					# this is ugly as hell but SearchForRecords can request
					# specific ownerids like this
				...
				# Alright, so this kinda sucks, but we have no good way of
				# automatically inserting
				# or updating the file's .size attribute, so we have to
				# manually check if any column
				# has that, and update it accordingly.
				# I have no idea if this will work with all games but it seems
				# to work in WarioWare.
				for i, col in enumerate(columns):
					attrcol = col + '___size'
			...
			# make sure users don't upload huge files, dunno what an actual
			# sensible maximum is
