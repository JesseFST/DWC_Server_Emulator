...
class GamespyDatabase(object):
	...
	def initialize_database(self):
		with Transaction(self.conn) as tx:
			...
			# TEXT is a good practice,
			# but I'm not good with databases and I'm not 100% positive that,
			# for instance, that all
			# user id's will be ints, or all passwords will be ints, etc,
	...
	def get_next_free_profileid(self):
		...
		# Instead, it's better to create a new row and return the sqlite ROWID
		# instead.
		with Transaction(self.conn) as tx:
	...
	def update_profile(self, profileid, field):
		...
		# FIXME: Possible security issue due to embedding an unsanitized string
	...
	def update_time(self, profileid):
		timenow = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
		with Transaction(self.conn) as tx:
			tx.nonquery("UPDATE users SET lastactive = ? WHERE profileid = ?", (timenow, profileid))
	...
	def console_abuse(self,postdata): # ONLY FOR WII CONSOLES
		# Check for non-legit Wii consoles
		if 'cfc' in postdata:
			if len(postdata["cfc"]) > 20:
				   return True

		elif 'csnum' in postdata:
			with Transaction(self.conn) as tx:
	...
	def get_next_available_userid(self):
		...
		if r == None or r['maxuser'] == None:
			return '0000000000002'#Because all zeroes means Dolphin.  Don't wanna get confused during debugging
								  #later.
		else:
	...
	def generate_authtoken(self, userid, data):
		...
		if "words" in data:
			data["words"] = gs_utils.base64_encode(data["words"])
	...
	def get_buddy(self, userProfileId, buddyProfileId):
		...
		if userProfileId != 0 and buddyProfileId != 0:
			with Transaction(self.conn) as tx:
				row = tx.queryone("SELECT * FROM buddies WHERE userProfileId = ? (AND buddyProfileId = ? OR openhost = 1)", (userProfileId, buddyProfileId))
				profile = self.get_dict(row)
		return profile
