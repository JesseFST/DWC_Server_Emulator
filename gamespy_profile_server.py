...
class PlayerSession(LineReceiver):
	...
	def rawDataReceived(self, data):
		try:
			...
			# In the case where command string is too big to fit into one read,
			# any parts that could not be successfully
			# parsed are stored in the variable remaining_message.  On the next
			# rawDataReceived command, the remaining
			...
			if len(data) > 0 and data[0] != '\\':
				...
				# there's no chance of it being valid.
				# Look for the first instance of \final\ and remove everything
				# before it.
			...
			def cmd_err(data_parsed):
				# Maybe write unknown commands to a separate file later so new
				# data can be collected more easily?
	...
	def perform_login(self, data_parsed):
		...
		if authtoken_parsed == None:
			...
			msg = gs_query.create_gamespy_message([('__cmd__', "error"),
				('__cmd_val__', ""),
				('err', '266'),
				('fatal', ''),
		...
		if profileid != None:
			...
			if self.sdkrevision == "11": # Used in Tatsunoko vs Capcom
				...
				msg = gs_query.create_gamespy_message([('__cmd__', "blk"),
				...
				msg = gs_query.create_gamespy_message([('__cmd__', "bdy"),
			...
			# Take the first 4 letters of gsbrcd instead of gamecd because they
			# should be consistent across game
			# regions.  For example, the US version of Metroid Prime Hunters
			# has the gamecd "AMHE" and the first 4 letters
			# of gsbrcd are "AMHE".  However, the Japanese version of Metroid
			# Prime Hunters has the gamecd "AMHJ" with
			# the first 4 letters of bsbrcd as "AMHE".  Tetris DS is the other
			# way, with the first 4 letters as the
			# Japanese version (ATRJ) while the gamecd is region specific (ATRE
			# for US and ATRJ for JP).
			# gameid is used to send all people on the player's friends list a
			# status updates, so don't make it region
			# specific.
	...
	def perform_updatepro(self, data_parsed):
		# Wii example:
		# \updatepro\\sesskey\199714190\firstname\Wii:2555151656076614@WR9E\partnerid\11\final\
	...
	def perform_bm(self, data_parsed):
		if data_parsed['__cmd_val__'] in ("1", "5", "102", "103"): # Message to/from clients?
			if "t" in data_parsed:
				...
				msg = gs_query.create_gamespy_message([('__cmd__', "bm"),
					('__cmd_val__', "1"),
					('f', self.profileid),
					('msg', dest_msg),])
	...
	def perform_addbuddy(self, data_parsed):
		...
		if buddy_exists == False:
			...
			# Trying to add someone who is already a friend.  Just send status
			# updates
			self.send_status_to_friends(newprofileid)
			self.get_status_from_friends(newprofileid)
	...
	def perform_authadd(self, data_parsed):
		# Sample:
		# \authadd\\sesskey\231587549\fromprofileid\217936895\sig\f259f26d3273f8bda23c7c5e4bd8c5aa\final\
	...
	def send_status_to_friends(self, buddy_profileid=None):
		# TODO: Cache buddy list so we don't have to query the database every
		# time
		self.buddies = self.db.get_buddy_list(self.profileid)
		...
		for buddy in buddy_list:
			if buddy['buddyProfileId'] in self.sessions:
				#self.log(logging.DEBUG, "Sending status to buddy id %s
				#(%s:%d): %s" % (str(buddy['buddyProfileId']),
				#self.sessions[buddy['buddyProfileId']].address.host,
				#self.sessions[buddy['buddyProfileId']].address.port, msg))
				self.sessions[buddy['buddyProfileId']].transport.write(bytes(msg))

	def get_status_from_friends(self, buddy_profileid=None):
		# This will be called when the player logs in.  Grab the player's buddy
		# list and check the current sessions to
		# see if anyone is online.  If they are online, make them send an
		# update to the calling client.
		self.buddies = self.db.get_buddy_list(self.profileid)
